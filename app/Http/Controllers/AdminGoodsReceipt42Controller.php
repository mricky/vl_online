...
class AdminGoodsReceipt42Controller extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		$columns = [];
		$columns[] = ["label" => "Barang ", 'required' => true, "name" => "product_id", 'type' => 'number', 'readonly' => true];
	...
	public function getEdit($id){
		...
		DB::table('goods_receipt')->where('id', $id)->update(['amount_of_good' => $totalItem]);
	...
	public function hook_after_edit($id)
	{
		...
		$gr = DB::table('goods_receipt')->where('id', $id)->first();
	    $this->goodReceiptRepository->syncPurchaseItemQty($gr->purchase_order_id); // po id 
	...
	public function hook_before_delete($id)
	{
		...
		$this->goodReceiptRepository->syncPurchaseItemQty($receive->purchase_order_id); // po id

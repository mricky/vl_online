...
class ProductRepository implements IProduct
{
    ...
    public function updateSalesStokLocation($salesId)
    {
        ...
        try {
            ...
            $this->inventoryTransactionOut($salesId, 'OUT');
            DB::commit();
        } catch (\Exception $e) {
            DB::rollback();
            dd($e->getMessage());
    ...
    public function inventoryTransactionOut($salesId, $inventoryType)
    {
        $sales = DB::table('sales_orders')->where('id', $salesId)->first();
            ...
            foreach ($salesDetails as $item) {
                ...
                $productLocation = ProductLocation::find($item['product_location_id']);
                 DB::table('inventory_transaction')->insert([
                    'sales_id' => $sales->id,
                    'inventory_transaction_type' => 'OUT',
                    'product_id' => $product->id,
                    'item_code' => $product->code,
                    'item_name' => $product->name,
                    'source' => 'Sales-Delivery',
                    'doc_no' => $sales->order_number,
                    'qty_out' => $item->qty,
                    'unit_cost' => $item->price,
                    'transaction_date' => Carbon::now(),
                    'warehouse' => $productLocation->wh_location_id == 1 ? 'WH/Stock' : 'Partner',
    ...
    public function updateStokLocation($id)
    {
        ...
        //dd($receives);
        // update po_vendor, vendor_name

        // pertama sudah di buat di otomatis di product location saat create good receipt otomatis
        // TODO : Check here
        try {
            DB::beginTransaction();

            ProductLocation::where('good_receipt_id', $id)->where('wh_location_id',1)->delete(); // gudang
            // Demand misal 10
            // qty in = 0
            ...
            foreach ($receives as $item) {
                ...
                DB::table('product_locations')->insert([
                    ...
                    'total' => $item['qty_in'] * $item['price'],
                    'is_automatic' => 0,
                    'description' => 'Auto transfer from vendor to internal stock',
                    'created_by' => CRUDBooster::myId() ?? 1,
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now()
                ]);

                // hitung ulang stok di vendor
                $productQty = DB::table('product_locations')->where('purchase_order_id', $purchaseOrder)
                    ->where('product_id', $item['product_id'])
                    ->where('wh_location_id', 1) // gudang
                    ->sum('qty_onhand');
              
                DB::table('product_locations')->where('purchase_order_id', $purchaseOrder)
                    ->where('product_id', $item['product_id'])
                    ->where('wh_location_id', 2)
                    ->update([
                        'qty_onhand' => DB::raw('qty_request-'.$productQty),
                        'total' => DB::raw('qty_onhand-'.$item['qty_in'].' * product_price'),
                        'created_by' => CRUDBooster::myId() ?? 1
                    ]);
